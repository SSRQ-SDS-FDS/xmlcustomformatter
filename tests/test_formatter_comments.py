...
class TestXMLCustomFormatterComments:
    ...
    def comments(self, request: FixtureRequest) -> tuple[str, str, str]:
        """Yields tets_name, xml_content, expected_result"""
        return cast(tuple[str, str, str], request.param)

    @pytest.fixture
    def xml_file(self, tmp_path: Path, comments: tuple[str, str, str]) -> str:
        """Writes the XML content to a temp file and returns the path as a string."""
        _, xml_content, _ = comments
        file_path = tmp_path / "input.xml"
        file_path.write_text(xml_content)
        return str(file_path)

    def test_xml_comment(self, comments: tuple[str, str, str], xml_file: str) -> None:
        """Tests Comment nodes are formatted correctly."""
        test_name, _, expected = comments
